...
short summary maintained in lockstep with it.
...
- [Runner CLI](/reference/runner-cli): the `trustacks rule new` /
  `test` / `lint` / `sign` flow.
- `trustacks-policy` repository: the full standard plus
  community-contributed framework packs. Apache 2.0; flips public at
